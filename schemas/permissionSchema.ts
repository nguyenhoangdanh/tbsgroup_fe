...
export const createPermissionSchema = z.object({
  ...
  name: z.string().min(1, 'Tên quyền là bắt buộc').max(100, 'Tên quyền không được vượt quá 100 ký tự'),
  description: z.string().max(500, 'Mô tả không được vượt quá 500 ký tự').optional(),
  type: z.nativeEnum(PermissionType, { errorMap: () => ({ message: 'Loại quyền không hợp lệ' }) }).default(PermissionType.PAGE_ACCESS),
