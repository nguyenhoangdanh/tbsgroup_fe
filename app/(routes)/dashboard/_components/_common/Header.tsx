...
import ThemeSwitcher from "@/components/common/layouts/admin/ThemeSwitcher";
...
const Header = () => {
  ...
  return (
    <header className="sticky top-0 z-50 flex h-16 items-center gap-4 !bg-[#208205] px-4 md:px-6">
      <nav
          ...
          {NAV_MENUS.map((menu, index) => (
            <li key={index} className="relative h-full">
              <Link
                href={menu.pathname}
                className={cn(
                  `text-white/90 text[16px] font-normal 
                                    z-[999] flex items-center px-3 justify-center
                    i               h-full transition-colors hover:text-opacity-90`,
                  {
              ...
              <div role="button" className="flex items-start gap-2">
                ...
                <div className="flex items-center gap-2">
                  <div className="grid flex-1 text-left text-sm leading-tight">
                    ...
                    <p className="truncate block w-full max-w-[150px] text-sx text-white/50">
                      {user?.email}
                    </p>
                  </div>
                  <ChevronDownIcon className="ml-auto size-4 text-white/80" />
                </div>
              </div>
            </DropdownMenuTrigger>
            <DropdownMenuContent>
