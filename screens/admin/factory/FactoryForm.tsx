...
const FactoryForm = memo(({ 
  ...
  const form = useForm<FactorySchema>({
    ...
    defaultValues: data
      ? {
          ...
          managingDepartmentId: data.managingDepartmentId || null,
        }
  ...
  const handleSubmit = useCallback(
    async (values: FactorySchema) => {
      ...
      try {
        ...
        if (onSubmit) {
          ...
          // If result is true, form submission was successful
          if (result === true) {
            console.log('Factory form submitted successfully');
            if (onSuccess) {
              onSuccess();
            }
          }
    ...
    [isReadOnly, isSubmitting, onSubmit, onSuccess],
  );
  ...
  const factoryDetails = data as FactoryWithDetails;
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FieldInput
            ...
            disabled={isSubmitting || isReadOnly || !!data?.id}
            required
          />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FieldSelect
            ...
            disabled={isSubmitting || isReadOnly || isLoadingDepartments}
