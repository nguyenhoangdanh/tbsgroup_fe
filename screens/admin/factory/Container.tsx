...
const FactoryManagementScreen: React.FC = React.memo(() => {
  ...
  const incrementOperationCount = useCallback((operationType = 'general') => {
    ...
    if (operationCountRef.current > MAX_OPERATIONS) {
      console.warn(
        `Exceeded maximum operations (${MAX_OPERATIONS}) in FactoryManagementScreen [${operationType}]`,
      );
      return false;
    }
  ...
  const stableFilters = useMemo(() => {
    ...
    if (
      filterTrackers.departmentType === 'HEAD_OFFICE' ||
      filterTrackers.departmentType === 'FACTORY_OFFICE'
    ) {
  ...
  const handleEditFactory = useCallback(
    async (factory: Factory): Promise<boolean> => {
  ...
  const handleFactoryFormSubmit = useCallback(
    async (data: FactoryCreateDTO | FactoryUpdateDTO): Promise<boolean> => {
      ...
      try {
        ...
        if ('id' in data && data.id) {
          const { id, ...updateData } = data as FactoryUpdateDTO & { id: string };
          await handleUpdateFactory(id, updateData as Omit<FactoryUpdateDTO, 'id'>);
        } else {
          const { ...createData } = data;
          await handleCreateFactory(createData as Omit<FactoryCreateDTO, 'id'>);
        }
  ...
  const viewFormComponent = useMemo(() => <FactoryForm />, []);
  ...
  useEffect(() => {
    if (factoryList?.data) {
      ...
      const withFactoryOffice = factoryList.data.filter(
        factory => factory.managingDepartmentId,
      ).length;
...
FactoryManagementScreen.displayName = 'FactoryManagementScreen';
