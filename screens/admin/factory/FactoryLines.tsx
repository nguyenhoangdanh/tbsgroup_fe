...
const FactoryLines: React.FC<FactoryLinesProps> = ({ factoryId }) => {
    ...
    // Line mutations
    const { createLineMutation, updateLineMutation, deleteLineMutation } = useLineMutations();
    ...
    const handleDeleteLine = useCallback((lineId: string) => {
        showDialog({
            ...
            description: 'Bạn có chắc chắn muốn xóa dây chuyền này không? Thao tác này không thể hoàn tác.',
            type: DialogType.DELETE,
            data: { lineId },
            onSubmit: async (data) => {
                try {
                    await deleteLineMutation.mutateAsync(data.lineId);
            ...
            <Card>
                ...
                <CardContent>
                    <div className="text-destructive">
                        Không tìm thấy nhà máy với ID đã cung cấp
                    </div>
                </CardContent>
                <CardFooter>
                ...
                <TabsContent value="lines" className="space-y-4">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Quản lý các dây chuyền sản xuất của nhà máy {factory.name}</CardDescription>
                        </CardHeader>
                        <CardContent>
                            <DataTable
                        ...
                        <CardContent>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div className="bg-muted rounded-lg p-4">
                                    <h3 className="text-sm font-medium text-muted-foreground mb-2">Tổng số dây chuyền</h3>
                                ...
                                <div className="bg-muted rounded-lg p-4">
                                    ...
                                    <p className="text-3xl font-bold">{lines?.filter(line => line.status === 'ACTIVE').length || 0}</p>
                                </div>
                                <div className="bg-muted rounded-lg p-4">
                                    <h3 className="text-sm font-medium text-muted-foreground mb-2">Đang bảo trì</h3>
                                    <p className="text-3xl font-bold">{lines?.filter(line => line.status === 'MAINTENANCE').length || 0}</p>
                                </div>
                            </div>
                                            ...
                                            <div key={line.id} className="flex flex-col">
                                                ...
                                                <div className="w-full bg-muted rounded-full h-2.5">
                                                    <div
                                                        ...
                                                        style={{ width: `${Math.min(100, (line.capacity || 0) / 10)}%` }}
                                                    ></div>
                                                </div>
                                            </div>
                                        ))}
