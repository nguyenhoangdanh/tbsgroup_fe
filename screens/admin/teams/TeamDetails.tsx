...
import { Users, ArrowLeft, Edit, Trash2, Settings, Factory } from 'lucide-react';
...
const ErrorFallback = ({
  ...
  <Card>
    ...
    <CardContent>
      <div className="text-destructive">{error.message || 'Đã xảy ra lỗi không xác định'}</div>
    </CardContent>
    <CardFooter>
...
export default function TeamDetails({ params }: TeamDetailsProps) {
  ...
  const leaders = useMemo(() => teamDetails?.leaders || [], [teamDetails?.leaders]);
        ...
        onSubmit: async () => {
          try {
            ...
            toast({
              ...
              description:
                error instanceof Error ? error.message : 'Đã xảy ra lỗi khi xóa tổ trưởng',
              variant: 'error',
            });
            setIsProcessing(false);
            return false;
  ...
  const isLoading =
    isLoadingTeam || isLoadingUsers || isTeamActionLoading || isRefreshing || isProcessing;
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="font-medium">
                      {teamDetails?.lineName || teamDetails?.lineId || 'Chưa xác định'}
                    </p>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="font-medium">
                      {teamDetails?.createdAt
                        ? new Date(teamDetails.createdAt).toLocaleString('vi-VN')
                        : 'N/A'}
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm">
                    {teamDetails?.updatedAt
                      ? new Date(teamDetails.updatedAt).toLocaleString('vi-VN')
                      : 'N/A'}
                  </p>
                </div>
              </CardContent>
        ...
        <AlertDialog open={confirmDelete} onOpenChange={setConfirmDelete}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Bạn có chắc chắn muốn xóa tổ `{teamDetails?.name}?` Hành động này không thể hoàn
                tác.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
