      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div
            ...
            onClick={() => handleViewTeam(row.original.id)}
          >
            {row.original.name}
          </div>
        ),
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-muted-foreground">Dây chuyền: {lineDetails?.name || lineId}</p>
            </div>
          </div>
        </div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <div className="flex space-x-2">
              <Button onClick={handleCreateTeam} variant="default" disabled={isLoading}>
                <PlusIcon className="mr-2 h-4 w-4" />
                Thêm tổ mới
              </Button>
              {selectedTeams.size > 0 && (
                <Button variant="destructive" onClick={handleBatchDeleteTeams} disabled={isLoading}>
