...
export default function TeamSettings({ params }: TeamSettingsProps) {
  ...
  const { updateTeam, invalidateTeamCache, isUpdating: isTeamUpdating, queries } = useTeam();
  ...
  const handleSettingUpdate = useCallback(
    async (updateData: Partial<TeamUpdateDTO>) => {
      ...
      try {
        ...
        toast({
          ...
          description:
            error instanceof Error ? error.message : 'Đã xảy ra lỗi khi cập nhật cài đặt',
          variant: 'error',
          duration: 5000,
        });
      } finally {
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-muted-foreground">
                {teamDetails?.name} ({teamDetails?.code}) - Dây chuyền:{' '}
                {teamDetails?.lineName || 'Đang tải...'}
              </p>
            </div>
          </div>
        </div>
              ...
              <CardContent className="space-y-6">
                <div className="space-y-4">
                  <div className="flex items-center justify-between py-4">
                    ...
                    <Button onClick={handleGoToMembers} variant="outline" disabled={isPageLoading}>
                      <Users className="mr-2 h-4 w-4" />
                      Quản lý thành viên
                    </Button>
                  </div>
