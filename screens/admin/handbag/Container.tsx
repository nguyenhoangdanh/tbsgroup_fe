...
import { HandBag, HandBagCreateDTO, HandBagUpdateDTO } from '@/common/interface/handbag';
...
const HandBagManagementScreen = memo(() => {
  ...
  const incrementOperationCount = useCallback((operationType = 'general') => {
    ...
    if (operationCountRef.current > MAX_OPERATIONS) {
      console.warn(
        `Exceeded maximum operations (${MAX_OPERATIONS}) in HandBagManagementScreen [${operationType}]`,
      );
      return false;
    }
  ...
  const handleDeleteHandBag = useCallback(
  ...
  const handleEditHandBag = useCallback(
    async (handBag: HandBag): Promise<boolean> => {
  ...
  const handleHandBagFormSubmit = useCallback(
    async (data: any): Promise<boolean> => {
      ...
      try {
        ...
        if (data.id) {
          const { id, ...updateData } = data as HandBagUpdateDTO & { id: string };
          await handleUpdateHandBag(id, updateData as Omit<HandBagUpdateDTO, 'id'>);
        } else {
          const createData: Omit<HandBagCreateDTO, 'id'> = data;
  ...
  const viewFormComponent = useMemo(() => <HandBagForm />, []);
...
HandBagManagementScreen.displayName = 'HandBagManagementScreen';
