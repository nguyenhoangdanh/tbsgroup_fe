...
import { BagColor, BagColorCreateDTO } from '@/common/interface/handbag';
...
const BagColorManagementScreen = React.memo(() => {
  ...
  const incrementOperationCount = useCallback((operationType = 'general') => {
    ...
    if (operationCountRef.current > MAX_OPERATIONS) {
      console.warn(
        `Exceeded maximum operations (${MAX_OPERATIONS}) in BagColorManagementScreen [${operationType}]`,
      );
      return false;
    }
  ...
  const fetchLatestBagColor = useCallback(
    async (id: string): Promise<BagColor | null> => {
      try {
  ...
  const handleDeleteBagColor = useCallback(
    ...
    [deleteBagColorMutation, safeRefetch, selectedBagColor?.id, isMounted, incrementOperationCount],
  );

  // Handle edit operations
  const handleEditBagColor = useCallback(
    async (bagColor: BagColor): Promise<boolean> => {
  ...
  const handleCreateBagColor = useCallback(
    async (data: BagColorCreateDTO): Promise<BagColor> => {
      ...
      try {
        ...
        //  Invalidate cache
        await onBagColorMutationSuccess(createdId, String(data.handBagId));
  ...
  const handleUpdateBagColor = useCallback(
    async (id: string, updateData: any): Promise<BagColor> => {
      ...
      try {
        ...
        //   Invalidate cache
        await onBagColorMutationSuccess(id, String(updateData.handBagId));
  ...
  const handleBagColorFormSubmit = useCallback(
    async (data: any): Promise<boolean> => {
      ...
      try {
        ...
        if (data.id) {
          ...
          await handleUpdateBagColor(String(id), updateData);
        } else {
    ...
    [handleCreateBagColor, handleUpdateBagColor, safeRefetch, isMounted, incrementOperationCount],
  );
  ...
  const columns: ColumnDef<BagColor>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (row.original.handBagId ? `Handbag ID: ${row.original.handBagId}` : '-'),
  ...
  const viewFormComponent = useMemo(() => <BagColorForm />, []);
...
BagColorManagementScreen.displayName = 'BagColorManagementScreen';
