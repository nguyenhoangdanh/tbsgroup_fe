...
const EditBagRates = () => {
    ...
    // Sử dụng URLSearchParams để lấy handBagId từ query string
    ...
    // Lấy handBagId từ query params khi component mount
    useEffect(() => {
        // Client-side: lấy handBagId từ URL
    ...
    // Debug console logs
    useEffect(() => {
        console.log("Current handBagId:", handBagId);
    }, [handBagId]);
    ...
    const fetchData = async () => {
        try {
            ...
            console.log("Fetching data for handBagId:", handBagId);

            // Fetch handbag details
            ...
            if (handBagResponse && handBagResponse.data) {
                setHandBag(handBagResponse.data);
            } else {
            ...
            if (ratesResponse && ratesResponse.data) {
                const ratesData = ratesResponse.data.map((rate: any) => ({
                    id: rate.id,
    ...
    // Check if a group is already selected in another field
    const isGroupSelected = (groupId: string, currentIndex: number) => {
        const values = form.getValues();
        return values.rates.some((rate, index) =>
            rate.groupId === groupId && index !== currentIndex
        );
    };
                        ...
                        <form onSubmit={form.handleSubmit(onSubmit)}>
                            ...
                            {fields.length === 0 ? (
                                <div className="text-center py-8 text-gray-500">
                                    Chưa có dữ liệu năng suất. Vui lòng nhấn "Thêm nhóm mới" để bắt đầu.
                                </div>
                            ) : (
