...
import { Loader2, AlertCircle, BarChart as BarChartIcon, PieChart as PieChartIcon, LineChart as LineChartIcon, AreaChart as AreaChartIcon, Activity } from "lucide-react";
                ...
                <CardHeader className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                    ...
                    <Tabs defaultValue="bar"
                        ...
                        <TabsList className="flex flex-wrap gap-2 w-full sm:w-auto">
                            <TabsTrigger value="bar" className="flex items-center gap-1 px-2 py-1 text-xs sm:text-sm">
                                <BarChartIcon className="w-3 h-3 sm:w-4 sm:h-4" /> Bar
                            </TabsTrigger>
                            <TabsTrigger value="area" className="flex items-center gap-1 px-2 py-1 text-xs sm:text-sm">
                                <AreaChartIcon className="w-3 h-3 sm:w-4 sm:h-4" /> Area
                            </TabsTrigger>
                            <TabsTrigger value="line" className="flex items-center gap-1 px-2 py-1 text-xs sm:text-sm">
                                <LineChartIcon className="w-3 h-3 sm:w-4 sm:h-4" /> Line
                            </TabsTrigger>
                            <TabsTrigger value="pie" className="flex items-center gap-1 px-2 py-1 text-xs sm:text-sm">
                                <PieChartIcon className="w-3 h-3 sm:w-4 sm:h-4" /> Pie
                            </TabsTrigger>
                            <TabsTrigger value="radar" className="flex items-center gap-1 px-2 py-1 text-xs sm:text-sm">
                        ...
                        {activeTab === 'radar' && (
                            <ResponsiveContainer width="100%" height="100%">
                                <RadarChart cx="50%" cy="50%" outerRadius="80%" data={sortedGroupData}>
                                    ...
                                    <PolarRadiusAxis angle={30} domain={[0, Math.ceil(analysisData.highestOutputRate * 1.1)]} />
                                    <Radar
                                        name="Năng suất"
                                        dataKey="outputRate"
                                        stroke="#4f46e5"
