...
const HandBagsList: React.FC<HandBagsListProps> = ({
  handBags: propHandBags,
...
const HandBagsListContent: React.FC<HandBagsListProps> = ({
  handBags: propHandBags,
  isLoading: propIsLoading,
}) => {
  ...
  const columns: ColumnDef<HandBagWithStats>[] = [
    ...
    },
    {
      ...
      header: 'Số nhóm',
      cell: ({ row }) => {
        return <div className="text-center">{row.getValue('totalGroups')}</div>;
      },
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => handleViewDetails(handBag.id)}>
