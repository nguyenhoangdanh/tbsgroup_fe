...
const LoadingRenderer: React.FC = () => {
  const { configs, stopLoading } = useLoading();

  const sortedEntries = React.useMemo(() => {
    return Object.entries(configs).sort(
      ([, configA], [, configB]) => (configB.priority || 0) - (configA.priority || 0),
    );
  }, [configs]);
...
const LoadingComponent: React.FC<LoadingComponentProps> = ({ id, config, onExitComplete }) => {
  ...
  switch (config.variant) {
    case 'fullscreen':
      return (
        <FullscreenLoader config={config} onExitComplete={onExitComplete} {...componentProps} />
      );
    case 'table':
      return (
        <TableSkeletonLoader config={config} onExitComplete={onExitComplete} {...componentProps} />
      );
    default:
      return null;
  }
