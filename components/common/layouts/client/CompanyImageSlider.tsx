  ...
  const renderBullets = () => {
    if (images.length <= 5) {
      ...
      return images.map((_, index) => (
        <button
          key={index}
          className={`w-2 h-2 sm:w-3 sm:h-3 mx-1 rounded-full focus:outline-none transition-all duration-300 ${
      ...
      for (let i = 0; i < 5; i++) {
        let indexToShow;
        if (i === 0)
          indexToShow = 0; // Bullet đầu tiên
        else if (i === 4)
          indexToShow = images.length - 1; // Bullet cuối cùng
        else {
          // 3 bullets ở giữa
      ...
      <div className="flex items-center justify-between p-2 bg-gray-200">
        ...
        <div className="flex items-center justify-center flex-grow">
          {images.map((_, index) => (
            <button
              key={index}
              className={`w-3 h-3 mx-1.5 rounded-full focus:outline-none transition-all duration-300 ${
                index === currentIndex ? 'bg-green-600 scale-125' : 'bg-gray-300 hover:bg-gray-400'
              }`}
              onClick={() => goToSlide(index)}
              aria-label={`Đến slide ${index + 1}`}
        ...
        {/* Auto-play control */}
        <button
          className={`flex items-center gap-1 px-2 py-1 rounded text-xs md:text-sm transition-all duration-300 ${
            isAutoPlaying ? 'bg-green-600 text-white' : 'bg-gray-300 text-black'
          }`}
