...
import { Check, ChevronsUpDown, CalendarIcon, ClockIcon, Eye, EyeOff, X } from 'lucide-react';
    ...
    return (
      <FieldContainer>
        ...
        >
          <SelectTrigger
            id={name}
            className={cn(inputClass, 'flex justify-between items-center')}
            aria-invalid={!!error}
            aria-describedby={error ? `${name}-error` : undefined}
          >
    ...
    return (
      <FieldContainer className={className}>
        ...
        <Popover open={comboboxOpen} onOpenChange={setComboboxOpen}>
          ...
          <PopoverContent
            className="w-full p-0 overflow-hidden border rounded-md z-50"
            align="start"
          >
                  ...
                  {filteredOptions.map(option => (
                    <CommandItem
                      ...
                      onSelect={currentValue => {
                        ...
                        const newValue = currentValue === String(field.value) ? '' : option.value;
...
export const DateTimeField = memo(
  <T extends FieldValues>({
    ...
    const { dateOpen, setDateOpen, timeOpen, setTimeOpen, handleDateSelect, handleTimeSelect } =
      useFormField({
        field,
        type: 'datetime',
      });
