...
export const useLineQueries = () => {
   ...
   */
  const getAccessibleLinesForUser = (options?: {
    enabled?: boolean;
    refetchOnWindowFocus?: boolean;
    staleTime?: number;
  }): UseQueryResult<Line[], Error> => {
    ...
    return useQuery<Partial<LineWithDetails>, Error>({
      ...
      queryFn: async () => {
        ...
        try {
          ...
          const [lineData, managersData] = await Promise.all([linePromise, managersPromise]);
  ...
  const updateLineCache = useCallback(
    (lineId: string, updatedData: Partial<Line>) => {
      ...
      if (factoryId) {
        ...
        queryClient.setQueriesData<Line[]>(
          { queryKey: ['factory', factoryId, 'lines'] },
          oldData => {
            if (!oldData) return oldData;
      ...
      queryClient.setQueriesData(
        { queryKey: ['line-list'] },
        (oldData: BaseResponseData<Line> | undefined) => {
      ...
      queryClient.setQueryData(
        ['line', lineId, 'details'],
        (oldData: Partial<LineWithDetails> | undefined) => {
          if (!oldData) return oldData;
          return {
            ...oldData,
            ...updatedData,
  ...
  const batchPrefetchLines = useCallback(
    async (lineIds: string[], includeManagers = false) => {
      ...
      for (let i = 0; i < lineIds.length; i += batchSize) {
        ...
        await Promise.all(batch.map(id => prefetchLineDetails(id, { includeManagers })));
