...
export const useLineMutations = () => {
  ...
  const createLineMutation = useMutation({
    ...
    onSuccess: (newLine, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['factory', factoryId, 'lines'] });
    ...
    onMutate: async (data) => {
      ...
      if (previousLine) {
        ...
        if (previousLine.factoryId) {
          queryClient.setQueriesData({ queryKey: ['factory', previousLine.factoryId, 'lines'] }, (oldData: Line[] | undefined) => {
            if (!oldData) return oldData;
            
            return oldData.map((line: Line) => 
              line.id === data.id ? updatedLine : line
            );
          });
        }
    ...
    onError: (err, data, context: any) => {
      ...
      if (context?.previousLine) {
        ...
        if (context.previousLine.factoryId) {
          queryClient.setQueriesData({ queryKey: ['factory', context.previousLine.factoryId, 'lines'] }, (oldData: Line[] | undefined) => {
            if (!oldData) return oldData;
  ...
  const deleteLineMutation = useMutation({
    ...
    onMutate: async (lineId) => {
      ...
      if (factoryId) {
        ...
        await queryClient.cancelQueries({ queryKey: ['factory', factoryId, 'lines'] });
        
        // Snapshot previous factory lines
        const previousFactoryLines = queryClient.getQueryData<Line[]>(['factory', factoryId, 'lines']);
   ...
   */
  const invalidateLineCaches = useCallback(async (lineId: string, options?: { forceRefetch?: boolean }) => {
    if (!lineId) return;
