...
export const useUserMutations = () => {
  ...
  const createUserMutation = useMutation({
    ...
    onMutate: async (newUserData) => {
      ...
      // Update each query with optimistic data
    ...
    onSuccess: () => {
      ...
      // Mark as stale without auto-refetch
  ...
  // Update user mutation (similar pattern)
  const updateUserMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<TUserSchema> }) => 
      UserService.update(id, data),
    // Similar optimistic update implementation...
  });
  
  // Delete user mutation (similar pattern)
