...
import { Team, TeamCreateDTO, TeamUpdateDTO, TeamLeaderDTO } from '@/common/interface/team';
...
export const useTeamMutations = () => {
  ...
  const updateTeamMutation = useMutation({
    mutationFn: (data: TeamUpdateDTO & { id: string }) => updateTeam(data.id, data),
    onMutate: async (data) => {
      ...
      if (previousTeam) {
        ...
        if (previousTeam.lineId) {
          queryClient.setQueriesData({ queryKey: ['line', previousTeam.lineId, 'teams'] }, (oldData: Team[] | undefined) => {
    ...
    onError: (err, data, context: any) => {
      ...
      if (context?.previousTeam) {
        ...
        if (context.previousTeam.lineId) {
          queryClient.setQueriesData({ queryKey: ['line', context.previousTeam.lineId, 'teams'] }, (oldData: Team[] | undefined) => {
            if (!oldData) return oldData;
  ...
  const deleteTeamMutation = useMutation({
    ...
    onMutate: async (teamId) => {
      ...
      if (lineId) {
        ...
        const previousLineTeams = queryClient.getQueryData<Team[]>(['line', lineId, 'teams']);
    ...
    onError: (err, teamId, context: any) => {
      ...
      if (context?.previousTeam) {
        queryClient.setQueriesData({ queryKey: ['team-list'] }, (oldData: any) => {
          ...
          const teamExists = oldData.data.some((team: Team) => team.id === teamId);
  ...
  const addLeaderMutation = useMutation({
    mutationFn: ({ teamId, leaderDTO }: { teamId: string; leaderDTO: TeamLeaderDTO }) => 
    ...
    onMutate: async ({ teamId, leaderDTO }) => {
      ...
      const previousLeaders = queryClient.getQueryData<any[]>(['team', teamId, 'leaders']) || [];
      ...
      const optimisticLeader: any = {
        ...
        startDate: typeof leaderDTO.startDate === 'string' 
          ? leaderDTO.startDate 
    ...
    onError: (err, { teamId }, context: any) => {
      ...
      if (context?.previousLeaders) {
        queryClient.setQueryData(['team', teamId, 'leaders'], context.previousLeaders);
  ...
  const removeLeaderMutation = useMutation({
    ...
    onMutate: async ({ teamId, userId }) => {
      ...
      const updatedLeaders = previousLeaders.filter(leader => leader.userId !== userId);
      queryClient.setQueryData(['team', teamId, 'leaders'], updatedLeaders);
    ...
    onMutate: async ({ teamId, userId, data }) => {
      ...
      const updatedLeaders = previousLeaders.map(leader => {
        if (leader.userId === userId) {
          return { 
            ...leader, 
            isPrimary: data.isPrimary !== undefined ? data.isPrimary : leader.isPrimary,
      ...
      if (context?.snapshotData?.length) {
        queryClient.setQueriesData({ queryKey: ['team-list'] }, (oldData: any) => {
          ...
          if (teamsToRestore.length) {
            ...
            const existingIds = new Set(oldData.data.map((team: Team) => team.id));
            const newTeamsToAdd = teamsToRestore.filter((team: Team) => !existingIds.has(team.id));
   ...
   */
  const invalidateTeamCaches = useCallback(async (teamId: string, options?: { forceRefetch?: boolean }) => {
    if (!teamId) return;
