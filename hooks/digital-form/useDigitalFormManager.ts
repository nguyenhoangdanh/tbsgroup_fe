...
import { TDigitalFormCreate, TDigitalFormUpdate } from '@/schemas/digital-form.schema';
...
export const useDigitalFormManager = () => {
  ...
  const crudHandlers = useDigitalFormCrudHandlers({
    ...
    onCreateForm: (data: TDigitalFormCreate) =>
      mutations.createFormMutation.mutateAsync(data).then(res => res.data?.id || null),
    onUpdateForm: (id: string, data: TDigitalFormUpdate) =>
      mutations.updateFormMutation.mutateAsync({ id, data }).then(() => true),
  ...
  const createNewForm = useCallback(
    async (formData: TDigitalFormCreate) => {
      setLoading(true);
      setError(null);
