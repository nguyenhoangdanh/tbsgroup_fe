...
import {
  ...
  getHandBagById as getHandBagByIdApi,
...
export const HandBagProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const { listItems: listHandBags, invalidateItemCache, invalidateListCache } = useHandBagQueries();
  ...
  const fetchLatestHandBag = useCallback(
    async (identifier: string, isName = false): Promise<HandBag | null> => {
      try {
        if (!isName) {
          try {
  ...
  const handleCreateHandBag = useCallback(
    async (data: Omit<HandBagCreateDTO, 'id'>): Promise<HandBag> => {
  ...
  const handleUpdateHandBag = useCallback(
    async (id: string, updateData: Omit<HandBagUpdateDTO, 'id'>): Promise<HandBag> => {
  ...
  const batchDeleteHandBagsMutation = useCallback(
    async (ids: string[]) => {
  ...
  return <HandBagContext.Provider value={contextValue}>{children}</HandBagContext.Provider>;
};
