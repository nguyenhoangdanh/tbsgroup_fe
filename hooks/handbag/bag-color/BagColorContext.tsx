...
import {
  ...
  getBagColorById as getBagColorByIdApi,
...
export const BagColorProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchLatestBagColor = useCallback(
    async (identifier: string, isName = false): Promise<BagColor | null> => {
      try {
        if (!isName) {
          try {
            ...
            console.log("Couldn't fetch by ID directly, trying search...", error);
          }
        }

        //  For name search, use direct query instead of hook
        const params: BagColorCondDTO & BasePaginationParams = {
          page: 1,
          limit: 1,
        };
  ...
  const handleCreateBagColor = useCallback(
    async (data: Omit<BagColorCreateDTO, 'id'>): Promise<BagColor> => {
  ...
  const handleUpdateBagColor = useCallback(
    async (id: string, updateData: Omit<BagColorUpdateDTO, 'id'>): Promise<BagColor> => {
  ...
  const batchDeleteBagColorsMutation = useCallback(
    async (ids: string[]) => {
  ...
  return <BagColorContext.Provider value={contextValue}>{children}</BagColorContext.Provider>;
};
