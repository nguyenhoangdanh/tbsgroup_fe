...
export const BagProcessProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const incrementOperationCount = useCallback((operationType = 'general') => {
    ...
    if (operationCountRef.current > MAX_OPERATIONS) {
      console.warn(
        `Exceeded maximum operations (${MAX_OPERATIONS}) in BagProcessContext [${operationType}]`,
      );
      return false;
    }
  ...
  const fetchLatestBagProcess = useCallback(
    async (id: string): Promise<BagProcess | null> => {
      try {
  ...
  const handleDeleteBagProcess = useCallback(
  ...
  const handleEditBagProcess = useCallback(
    async (bagProcess: BagProcess): Promise<boolean> => {
  ...
  const handleCreateBagProcess = useCallback(
    async (data: any): Promise<BagProcess> => {
    ...
    [
      createBagProcessMutation,
      onBagProcessMutationSuccess,
      fetchLatestBagProcess,
      incrementOperationCount,
    ],
  );

  // Memoized update handler with proper operation tracking
  const handleUpdateBagProcess = useCallback(
    async (id: string, updateData: any): Promise<BagProcess> => {
  ...
  const handleBagProcessFormSubmit = useCallback(
  ...
  return <BagProcessContext.Provider value={contextValue}>{children}</BagProcessContext.Provider>;
};
