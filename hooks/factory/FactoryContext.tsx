...
import { Factory, FactoryCreateDTO, FactoryUpdateDTO, FactoryCondDTO, FactoryManagerDTO } from "@/common/interface/factory";
...
import { batchDeleteFactoriesParallel, getFactoryById as getFactoryByIdApi } from "@/apis/factory/factory.api";
...
export const FactoryProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const invalidateRelevantCaches = useCallback(async (factoryId: string, options = { refetch: true }) => {
    ...
    const manageOperation = useCallback(<T,>(
        operationKey: string,
        operation: () => Promise<T>
    ): Promise<T> => {
    ...
    const fetchLatestFactory = useCallback(async (factoryId: string): Promise<Factory | null> => {
        try {
    ...
    const handleCreateFactory = useCallback(async (data: Omit<FactoryCreateDTO, 'id'>): Promise<Factory> => {
    ...
    const handleUpdateFactory = useCallback(async (id: string, updateData: Omit<FactoryUpdateDTO, 'id'>): Promise<Factory> => {
    ...
    const handleAddManager = useCallback(async (factoryId: string, managerDTO: FactoryManagerDTO): Promise<void> => {
    ...
    const handleRemoveManager = useCallback(async (factoryId: string, userId: string): Promise<void> => {
    ...
    const batchDeleteFactoriesMutation = useCallback(async (ids: string[]) => {
