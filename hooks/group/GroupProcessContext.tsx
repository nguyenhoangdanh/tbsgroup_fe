...
export const GroupProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const { listGroups, getGroupById, invalidateGroupCache, invalidateGroupsCache } =
    useGroupQueries();

  const { createGroupMutation, updateGroupMutation, deleteGroupMutation, onGroupMutationSuccess } =
    useGroupMutations();
  ...
  const incrementOperationCount = useCallback((operationType = 'general') => {
    ...
    if (operationCountRef.current > MAX_OPERATIONS) {
      console.warn(
        `Exceeded maximum operations (${MAX_OPERATIONS}) in GroupContext [${operationType}]`,
      );
      return false;
    }
  ...
  const handleUpdateGroup = useCallback(
    async (id: string, updateData: any): Promise<Group> => {
  ...
  const handleGroupFormSubmit = useCallback(
    ...
    [handleCreateGroup, handleUpdateGroup, safeRefetch, isMounted, incrementOperationCount],
  );
