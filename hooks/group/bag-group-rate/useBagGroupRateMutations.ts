...
export const useBagGroupRateMutations = () => {
  ...
  const bagGroupRateMutations = useBaseMutations<
    BagGroupRate,
    BagGroupRateCreateDTO,
    BagGroupRateUpdateDTO
  >('bag-group-rate', createBagGroupRate, updateBagGroupRate, deleteBagGroupRate);
  ...
  const batchCreateBagGroupRatesMutation = useMutation({
    ...
    onMutate: async newData => {
      ...
      await queryClient.cancelQueries({
        queryKey: ['bag-group-rates-for-handbag', newData.handBagId],
      });

      //  Lưu trữ dữ liệu truy vấn hiện tại
      const previousListData = queryClient.getQueryData(['bag-group-rate-list']);
      const previousHandBagData = queryClient.getQueryData([
        'bag-group-rates-for-handbag',
        newData.handBagId,
      ]);
    ...
    onError: (error, variables, context) => {
      ...
      if (context?.previousHandBagData && context.handBagId) {
        queryClient.setQueryData(
          ['bag-group-rates-for-handbag', context.handBagId],
          context.previousHandBagData,
        );
      }
    },
  });
