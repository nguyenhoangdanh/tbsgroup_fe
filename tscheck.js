...
if (existingDirectories.length === 0) {
  console.error(
    '\x1b[31m%s\x1b[0m',
    'Không tìm thấy thư mục mã nguồn nào để kiểm tra.',
  );
  process.exit(1);
}
...
console.log(
  '\x1b[36m%s\x1b[0m',
  '📂 Đang kiểm tra các thư mục:',
  existingDirectories.join(', '),
);
...
console.log(
  '\x1b[36m%s\x1b[0m',
  `🔍 Đang chạy TypeScript check: ${tscCheckCommand}`,
);
    ...
    exec(tscCheckCommand, (error, stdout, stderr) => {
      ...
      if (error) {
        ...
        if (errorMatch && errorMatch[1]) {
          console.error(
            '\x1b[31m%s\x1b[0m',
            `Tìm thấy ${errorMatch[1]} lỗi TypeScript.`,
          );
        }
        ...
        console.log(
          '\x1b[32m%s\x1b[0m',
          '✅ Kiểm tra TypeScript thành công! Không tìm thấy lỗi nào.',
        );
        resolve();
      }
    });
...
if (isWatch) {
  ...
  Promise.all([
    ...
    .catch(() => {
      console.error(
        '\x1b[31m%s\x1b[0m',
        '❌ Một hoặc nhiều kiểm tra thất bại.',
      );
      process.exit(1);
    });
}
