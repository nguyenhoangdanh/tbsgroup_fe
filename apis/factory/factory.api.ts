...
export const getFactoriesList = async (
  ...
  if (params.managingDepartmentId)
    queryParams.append('managingDepartmentId', params.managingDepartmentId);
 ...
 * Tạo mới một nhà máy
 */
export const createFactory = async (data: FactoryCreateDTO): Promise<{ id: string }> => {
 ...
 */
export const updateFactory = async (id: string, data: FactoryUpdateDTO): Promise<void> => {
  const response = await fetchWithAuth(`/factories/${id}`, {
    method: 'PATCH',
    body: JSON.stringify(data),
  });
 ...
 */
export const getFactoryManagers = async (
  factoryId: string,
): Promise<
  {
    userId: string;
 ...
 */
export const removeFactoryManager = async (factoryId: string, userId: string): Promise<void> => {
  const response = await fetchWithAuth(`/factories/${factoryId}/managers/${userId}`, {
    method: 'DELETE',
  });
